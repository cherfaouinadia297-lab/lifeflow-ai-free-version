...
function StatsPage() {
  ...
  const weekData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      arr.push({
        day: i18n.formatDate(d, { weekday: "short", year: undefined, month: undefined, day: undefined }),
        pct,
        done,
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">{t("stats.title")}</h1>
          <p className="text-sm text-muted-foreground">{t("stats.subtitle")}</p>
        </div>

        {/* Key stats */}
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <KpiCard icon={<Trophy className="h-5 w-5" />} label={t("stats.kpi.level")} value={state.progress.level} accent />
          <KpiCard icon={<Sparkles className="h-5 w-5" />} label={t("stats.kpi.xp")} value={state.progress.xp} />
          <KpiCard icon={<Flame className="h-5 w-5" />} label={t("stats.kpi.streak")} value={state.progress.streak} />
          <KpiCard icon={<Target className="h-5 w-5" />} label={t("stats.kpi.completed")} value={totalCompleted} />
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <div className="text-xs text-muted-foreground">{t("stats.progressToLevel", { n: state.progress.level + 1 })}</div>
              <div className="font-display text-lg font-bold">{xpInLevel} / {XP_PER_LEVEL} XP</div>
            </div>
            <div className="text-xs text-muted-foreground">
              {overdue > 0 && <span className="text-destructive">{t("stats.overdue", { n: overdue })}</span>}
            </div>
          </div>
        ...
        {/* Weekly completion */}
        <ChartCard title={t("stats.weekly")}>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={weekData}>
              ...
              <Tooltip
                ...
                formatter={(v: number) => [`${v}%`, t("stats.completionLabel")]}
              />
        ...
        {/* XP trend */}
        <ChartCard title={t("stats.xpTrend")}>
          <ResponsiveContainer width="100%" height={240}>
        ...
        {/* Category split */}
        {catData.length > 0 && (
          <ChartCard title={t("stats.catSplit")}>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-bold">{t("stats.badges")}</h2>
          </div>
            ...
            {BADGES.map((b) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 text-xs font-bold">{t(`badge.${b.id}`)}</div>
                </div>
              );
            })}
          </div>
...
function EmptyState({ t }: { t: (k: string, v?: Record<string, string | number>) => string }) {
  return (
    <div className="rounded-2xl border-2 border-dashed border-border bg-card/50 p-8 text-center">
      ...
      <p className="mt-2 font-display text-base font-bold">{t("stats.empty")}</p>
      <p className="mt-1 text-sm text-muted-foreground">{t("stats.emptyHint")}</p>
    </div>
  );
}